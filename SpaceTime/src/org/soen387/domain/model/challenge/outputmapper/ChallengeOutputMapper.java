package org.soen387.domain.model.challenge.outputmapper;
...
public class ChallengeOutputMapper extends GenericOutputMapper<Long, Challenge> {
	...
	public static void insertStatic(Challenge c) throws SQLException {
		ChallengeTDG.insert(c.getId(), c.getVersion(), c.getChallenger()
				.getId(), c.getChallengee().getId(), c.getStatus().name());
	}

	public static void updateStatic(IChallenge c) throws SQLException,
			LostUpdateException {
		int count = ChallengeTDG.update(c.getId(), c.getVersion(),
				c.getChallenger().getId(), c.getChallengee().getId(), 
				c.getStatus().name());
		
		if (count == 0) {
			throw new LostUpdateException(
					"Lost Update editing challenge with id " + c.getId());
		}
		
		c.setVersion(c.getVersion() + 1);
	}

	public static void deleteStatic(IChallenge c) throws SQLException,
			LostUpdateException {
		int count = ChallengeTDG.delete(c.getId(), c.getVersion());
		if (count == 0) {
			throw new LostUpdateException(
					"Lost Update deleting challenge with id " + c.getId());
		}
	}
