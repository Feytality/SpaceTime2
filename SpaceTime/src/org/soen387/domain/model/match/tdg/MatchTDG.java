...
public class MatchTDG {
	...
	public static final String INSERT = "INSERT INTO " + TABLE_NAME + " ("
			+ COLUMNS + ") " + "VALUES (?,?,?,?);";
	public static int insert(long id, long version, long firstTeam,
			long secondTeam, String status) throws SQLException {
		Connection con = DbRegistry.getDbConnection();
	...
	public static final String UPDATE = "UPDATE " + TABLE_NAME
			+ " set version = version+1, firstTeam=?, secondTeam=?, status=? "
			+ " WHERE id=? AND version=?;";
	public static int update(long id, long version, long firstTeam,
			long secondTeam, String status) throws SQLException {
		Connection con = DbRegistry.getDbConnection();
