package org.soen387.domain.model.team.inputmapper;
...
import org.soen387.domain.model.team.finder.TeamFinder;

public class TeamInputMapper {
	
	public static Team find(long id) throws SQLException,
			MissingMappingException, MapperException {
		if (IdentityMap.has(id, Team.class))
			return IdentityMap.get(id, Team.class);
		...
		throw new DomainObjectNotFoundException(
				"Could not create a Team with id \"" + id + "\"");
	}

	public static Team find(IPlayer player) throws SQLException,
			MissingMappingException, MapperException {
		...
		throw new DomainObjectNotFoundException(
				"Could not create a Team from Player with id \""
						+ player.getId() + "\"");
	}
	...
	private static Team buildTeam(ResultSet rs) throws SQLException,
		...
		return new Team(id, version, name, player, pilots);
	}
