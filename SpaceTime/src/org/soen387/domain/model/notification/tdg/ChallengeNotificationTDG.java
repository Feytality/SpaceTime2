...
public class ChallengeNotificationTDG {
	...
	public static int insert(long id, long version, long recipient, boolean seen)
			throws SQLException {
		Connection con = DbRegistry.getDbConnection();
	...
	public static final String UPDATE = "UPDATE " + TABLE_NAME
			+ " set version = version+1, recipient=?, seen=? "
			+ " WHERE id=? AND version=?;";

	public static int update(long id, long version, long recipient, boolean seen)
			throws SQLException {
		Connection con = DbRegistry.getDbConnection();
