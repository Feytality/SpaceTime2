package org.soen387.domain.model.pilot.outputmapper;
...
public class PilotOutputMapper extends GenericOutputMapper<Long, Pilot> {
	...
	public static void insertStatic(IPilot p) throws SQLException {
		PilotTDG.insert(p.getId(), p.getVersion(), p.getName(), p.getPlayer()
				.getId());
	}

	public static void updateStatic(IPilot p) throws SQLException,
			LostUpdateException {
		int count = PilotTDG.update(p.getId(), p.getVersion(), p.getName(), p
				.getPlayer().getId());
		if (count == 0)
	...
	public static void deleteStatic(IPilot p) throws SQLException,
			LostUpdateException {
		...
		// Also remove pilot from the team.
